...
void mergeSortRange(int a[], int temp[], int left, int right) {
    if (left < right) {
        int middle = left + (right - left) / 2;

        mergeSortRange(a, temp, left, middle);
        mergeSortRange(a, temp, middle + 1, right);
...
int main () {
    ...
    // run timed sorts with sorted data
    keepgoing = 1;
    while (keepgoing) {
    ...
    // run timed sorts with reverse sorted data
    keepgoing = 1;
    while (keepgoing) {
